...
- Create new class definitions for custom functionality
...
We'll start by adding custom endpoints, both OData and non-OData, by simply extending the partial class `Presentation.Api.OData.CurrenciesController.g.cs` that have been generated.
...
We can create custom endpoints that are not generated from entities defined in our solution.

Again, both OData and non-OData endpoints are supported.

The code snippet below shows an example of how we can create both OData and non-OData endpoints for the `Application/Dto/TokenTypeDto` custom DTO we created in [this step](https://noxorg.dev/en/nox-lib-custom-dtos/#create-a-custom-dto):
